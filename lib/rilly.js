...
// unread is a flag indicating the item should be placed into the unread list (all new items should be unread/unsaved)
...
function Rilly(options) {
  var baseURL = "https://readitlaterlist.com/v2/";
	...
	this.store.createTable("pages", {url: "text", title: "text", text: "text", unsaved: "integer", unread: "integer"}, function(){
	  this.store.read("pages", {unread: true}, null, function(rows){
	    if(rows.length)
